...
function writeReturnhref()
{
	...
	if("0" == isIPv4){
		...
		if(lanip == "N/A")	
				httpip = "http://192.168.1.1/cgi-bin/net-tr069.asp";
		else
				httpip = "http://" + lanip + "/cgi-bin/net-tr069.asp" ;
	}
	document.write('<A href="' + httpip + '" onclick="resetCookie();">返回远程管理页面</A>');
}
...
<%else%>			
				tdResultElement.innerHTML = '<P><img src="/img/iconFail.jpg"><strong><font color="#FF0000" size="2">在ITMS上注册失败！请3分钟后重试，如无法解决请联系客户经理或拨打10000</font></strong></P>';
<% end if %>
		}
<% if tcWebApi_get("WebCustom_Entry", "isCTFJSupported", "h") = "Yes"  then %>				
		}
...
<FORM id="SelfCheckForm" name="SelfCheckForm" method="post" action="/cgi-bin/selfcheck.asp">
	<input type="hidden" id="stb_test" name="stb_test" value="0">
</FORM>

<FORM id="ConfigForm" name="ConfigForm" method="post">
<% if tcWebApi_get("WebCustom_Entry", "isCTFJSupported", "h") = "Yes"  then %>
	<INPUT TYPE="HIDDEN" ID="ReDefaultbootflg" NAME="ReDefaultbootflg" VALUE="0">
	<INPUT TYPE="HIDDEN" ID="restoreflag2" NAME="restoreflag2" VALUE="">
<% end if%>	
	<INPUT TYPE="HIDDEN" ID="Register_CHANGE" NAME="Register_CHANGE" VALUE="0">
	<INPUT TYPE="HIDDEN" ID="tryTimesflg" NAME="tryTimesflg" VALUE="-1">
	<INPUT TYPE="HIDDEN" ID="tryTimes" NAME="tryTimes" VALUE="-1">
  ...
  <input id="result_startvalue" name="result_startvalue" type="hidden" value="99">
...
<% end if%>
    ...
    <TD colSpan=2>
      <TABLE height="15%" width="100%" border=0>
        ...
        <TR>
<% if tcWebApi_get("WebCustom_Entry", "isCTSCSupported", "h") = "Yes"  then %>   
          <TD width="20%" align=right ><INPUT id="btnOK" name="btnOK1" style="WIDTH: 60px;HEIGHT: 20px" onclick="onOK()" type=button value="确定"></TD>
<%else%>
					<TD width="45%" align=right ><INPUT id="btnOK" name="btnOK2" style="WIDTH: 60px;HEIGHT: 20px" onclick="onOK()" type=button value="确定"></TD>
<% end if%>         
<% if tcWebApi_get("WebCustom_Entry", "isCTPONC9Supported", "h") = "Yes"  then%> 
					<TD width="10%"><INPUT id="btnSetting" name="btnSetting" style="WIDTH: 70px;HEIGHT: 20px" onclick="onSetting()" type=button value="高级设置"></TD>
<%else%>
          <TD width="10%">&nbsp;</TD>
<% end if%>
<% if tcWebApi_get("WebCustom_Entry", "isCTSIMCARDSEPARATIONSupported", "h") = "Yes"  then 
   if tcWebApi_get("SimCard_Entry", "cardExist", "h") = "1"  then %>
          <TD width="45%" align=left ><INPUT id="btnCancel" name="btnCancel" style="WIDTH: 60px;HEIGHT: 20px" onclick="onReset()" type="button" value="重置"></TD></TR></TBODY></TABLE></TD></TR>
   ...
   <%else%>
          <TD width="45%" align=left ><INPUT id="btnCancel" name="btnCancel_one" style="WIDTH: 60px;HEIGHT: 20px" onclick="onReset()" type="button" value="重置"></TD></TR></TBODY></TABLE></TD></TR></TBODY></TABLE></TD></TR></TABLE></DIV>
   <% end if
else%>
<% if tcWebApi_get("WebCustom_Entry", "isCTSCSupported", "h") = "Yes"  then %>  
          <TD width="20%" align=left ><INPUT id="btnCancel" name="btnCancel_two" style="WIDTH: 60px;HEIGHT: 20px" onclick="onReset()" type="button" value="重置">
<%else%>
					<TD width="45%" align=left ><INPUT id="btnCancel" name="btnCancel_three" style="WIDTH: 60px;HEIGHT: 20px" onclick="onReset()" type="button" value="重置">
<% end if%>
<% if tcWebApi_get("WebCustom_Entry", "isCTSCSupported", "h") = "Yes"  then %>    
          <TD width="20%" align=left ><INPUT id="btnCancel" name="btnCancel_four" style="WIDTH: 65px;HEIGHT: 20px" onclick="btnselfCheck()" type="button" value="终端自检">       	
<% end if%></TD>
<% if tcWebApi_get("WebCustom_Entry", "isCTSCSupported", "h") = "Yes"  then %>
					<TD width="20%" align=left ><INPUT id="btnCancel" name="btnCancel_five" style="WIDTH: 65px;HEIGHT: 20px" onclick="restore()" type="button" value="恢复出厂"></TD>
<% end if%> 
...
<% end if%>
  ...
  <TR id=trReturn style="DISPLAY: none">
    <TD align=middle colSpan=2><INPUT id=btnReturn name=btnReturn style="WIDTH: 60px;HEIGHT: 20px" onclick=onReturn(0); type=button value=返回> 
    </TD></TR>
...
<% if tcWebApi_get("WebCustom_Entry", "isCTFJSupported", "h") = "Yes"  then %>
    ...
    <TD colSpan=2>
      <TABLE height="15%" width="100%" border=0>
        <TBODY>
        <TR>
          <TD width="45%" align=right><INPUT id=btnReturn1 name=btnReturn_one onclick=onReturn(0); type=button value='返 回'> </TD>
          <TD width="10%">&nbsp;</TD>
          <TD width="45%" align=middle><INPUT type="button" value="恢复出厂设置"  id="buttondefaultbak" name="buttondefaultbak"  onClick="ReDefaultboot();" ></TD>
        </TR></TBODY></TABLE></TD>
  </TR>
  ...
  <TR id="submitbutton" style="DISPLAY: none">
    ...
    <TD colSpan=2>
      <TABLE height="15%" width="100%" border=0>
        <TBODY>
        <TR>
          <TD width="45%" align=right ><INPUT id="btnSubmit" name="btnSubmit" style="WIDTH: 60px;HEIGHT: 20px" onclick="onSubmit()" type=button value="提交"></TD>
          <TD width="10%">&nbsp;</TD>
          <TD width="45%" align=left><INPUT id="btnCancel" name="btnCancel_exp" style="WIDTH: 60px;HEIGHT: 20px" onclick="onCancel()" type="button" value="取消"></TD></TR></TBODY></TABLE></TD></TR>
<%else%>
...
<%else%>
			return "在OLT上注册失败，请检查光纤是否已正常连接、宽带识别码和密码是否正确，如无法解决请联系客户经理或拨打10000";		
<% end if%>
<% end if%>
<% end if%>
		}
...
<% else %>
					var tmp = "在ITMS上注册失败！请检查宽带识别码和密码是否正确，如无法解决请联系客户经理或拨打10000";
<% end if%>
<% if tcWebApi_get("WebCustom_Entry", "isCTSCSupported", "h") = "Yes"  then %>
...
<%else%>
					return "在ITMS上注册失败！请3分钟后重试或联系客户经理或拨打10000";
<%end if%>
				}
			}
			else if (rstatus == "2")
...
<%else%>
				return "在ITMS上注册超时！请检查线路后重试，如无法解决请联系客户经理或拨打10000";
<% end if%>
			}
			else if (rstatus == "5")
			{
...
<% end if%>
						...
						infoStr = "ITMS平台业务数据下发成功，共下发了" + serviceList.toString() + " " + serviceNum.toString() + "个业务，欢迎使用悦me";
<% if tcWebApi_get("WebCustom_Entry", "isCTSCSupported", "h") = "Yes"  then %>
...
<% if tcWebApi_get("WebCustom_Entry", "isCTJOYME2Supported", "h") = "Yes"  then %>
									if ( "8" == acsresult )
										tdResultElement.innerHTML = '<P><img src="/img/iconFail.jpg"><strong><font color="#FF0000" size="2">在ITMS上注册超时！请检查线路后重试，如无法解决请联系客户经理或拨打10000</font></strong></P>';
<%end if%>
								}
							}				
							document.cookie = "RegPageStatus=1";
