...
<%end if%>	
	...
	var vVpi = CycleV.split(',');
	/*num 21*/
	var CycleV = "<% tcWebApi_get("WanInfo_Common","CycleValue","s") %>";
	var vVci = CycleV.split(',');
...
function check_vpi_vci(atmVpi,atmVci)
{
	for(var i=0; i< (CurWan.length-1); i++)
	{
		if ( false == AddFlag && document.ConfigForm.curSetIndex.value == CurWan[i].domain )
			continue;
			
		if ( CurWan[i].vpi == atmVpi && CurWan[i].vci == atmVci)
		{
			alert("<%tcWebApi_get("String_Entry","AlreadyUsedText","s")%>");
			return false;
		}
	}
	return true;
...
function CheckForm(type)
{
	...
	with (getElById('ConfigForm'))
	{
		...
		if ( "ATM" == DslModeST.value )
		{
				...
				if(!check_vpi_vci(atmVpi.value,atmVci.value))
					return false;
						...
						<TD>
							<LABEL> 
								...
								<SELECT id=transMode name="transMode">
									...
									<OPTION value="XDSL" <%if tcWebApi_get("Wan_Common","TransMode","h") <> "Ethernet" then asp_Write("selected") end if%>>XDSL
								</SELECT>
							</LABEL>
						</TD>
...
function WanIndexConstruction(domain,WanName,vpi,vci)
{
	this.domain = domain;
...
function CheckWansActives()
{
	...
	{	
		vcurLinks[nCurTemp++] = new WanIndexConstruction(vEntryIndex[i], vEntryName[i],vVpi[i],vVci[i]);
	}
...
<%if tcWebApi_get("WebCustom_Entry", "isCTADTJSupported", "h") = "Yes" then%>
            <SELECT id=serviceList onchange=onSelectSvrList() name=serviceList> 
						...
						<OPTION value="VOICE" <%if tcWebApi_get("WanInfo_WanIF","ServiceList","h") = "VOICE" then asp_Write("selected") end if %>>VOICE
						<OPTION value="VOICE_INTERNET" <%if tcWebApi_get("WanInfo_WanIF","ServiceList","h") = "VOICE_INTERNET" then asp_Write("selected") end if %>>VOICE_INTERNET
						<OPTION value="TR069_VOICE" <%if tcWebApi_get("WanInfo_WanIF","ServiceList","h") = "TR069_VOICE" then asp_Write("selected") end if %>>TR069_VOICE
						<OPTION value="TR069_VOICE_INTERNET" <%if tcWebApi_get("WanInfo_WanIF","ServiceList","h") = "TR069_VOICE_INTERNET" then asp_Write("selected") end if %>>TR069_VOICE_INTERNET
