...
if Request_Form("Wan_Flag") = "1" then
	...
	if Request_Form("wanMode") = "Route" then
		...
		if Request_Form("linkMode") = "linkPPP" then
			...
			TCWebApi_set("system_entry","srvName","pppServerName")
	...
	if tcwebApi_get("WebCustom_Entry","isDefaultRouteManualSupported","h" ) = "Yes" then
		tcWebApi_set("Wan_Common","DefRouteManualIndex","defaultRoute")
	end if
...
function checkLoginUser()
{
	...
	setDisplay('td_enblService',0);
	setDisplay('divIpVersion',0);
	setDisplay('mulvidsec',0);
	setDisplay('secNat',0);
	setDisplay('secDefaultRoute',0);
	<% end if %>
}
...
<% end if %>
<% end if %>
			if((wanMode.value == "Bridge") || ("IPv6" == ipVer))
			{
				setDisplay('secNat',0);
			}
			else 
...
<% if tcwebApi_get("WebCustom_Entry","isDefaultRouteManualSupported","h") = "Yes" then %>
	var DefRouteManualIndex =  "<% tcWebApi_get("Wan_Common","DefRouteManualIndex","s") %>";
	var CurIFIndex = "<% tcWebApi_get("WanInfo_Common","CurIFIndex","s") %>";

	if(DefRouteManualIndex == CurIFIndex){
		console.log("===> 781");
		cb_defaultRoute[0].checked = true;
	}else{
		console.log("===> 784");
		cb_defaultRoute[1].checked = true;
	}

	if(wanMode.value == "Bridge"){
		setDisplay('secDefaultRoute',0);
		if(cb_defaultRoute[0].checked){
			cb_defaultRoute[1].click();
		}
	}else{
		//TR069_INTERNET VOICE_INTERNET TR069_VOICE_INTERNET
		if(serviceList.value == "INTERNET" || serviceList.value == "TR069_INTERNET" ||  serviceList.value == "VOICE_INTERNET" || serviceList.value == "TR069_VOICE_INTERNET"){
			<% if tcWebApi_get("WebCurSet_Entry","CurrentAccess","h") = "0" then %>
				setDisplay('secDefaultRoute',1);
			<% else %>
				setDisplay('secDefaultRoute',0);
			<% end if %>
		}else{
...
<% if tcwebApi_get("WebCustom_Entry","isDefaultRouteManualSupported","h") = "Yes" then %>
		...
		var DefRouteManualIndex =  "<% tcWebApi_get("Wan_Common","DefRouteManualIndex","s") %>";
		var CurIFIndex = "<% tcWebApi_get("WanInfo_Common","CurIFIndex","s") %>";
		defaultRoute.value = DefRouteManualIndex;
		console.log("===> 4489");
		cb_defaultRoute[1].click();
<% end if %>		
...
<% if tcwebApi_get("WebCustom_Entry","isDefaultRouteManualSupported","h") = "Yes" then %>	
	...
	if(DefRouteManualIndex == CurIFIndex){
		vForm.defaultRoute.value = "N/A";
	}else{
		vForm.defaultRoute.value = DefRouteManualIndex;
	}
<%end if%>
...
<% end if %>
                                ...
                                <TD id="td_enblService"><%tcWebApi_get("String_Entry","EnableText","s")%>
                                  <LABEL>
...
<% if tcwebApi_get("WebCustom_Entry","isDefaultRouteManualSupported","h") = "Yes" then %>
                  ...
                  <TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
                    <TBODY>
                    <TR>
                      <TD width=150><%tcWebApi_get("String_Entry","DefaultRouteText","s")%></TD>
                      <TD><LABEL>
                                    <INPUT id=cb_defaultRoute type=radio name=cb_defaultRoute onClick="DefaultRouteClick()">Enable &nbsp;&nbsp;
                                    <INPUT id=cb_defaultRoute type=radio name=cb_defaultRoute onClick="DefaultRouteClick()">Disable &nbsp;&nbsp;
                                    <INPUT id=defaultRoute type=hidden value="<% tcWebApi_get("Wan_Common","DefRouteManualIndex","s") %>" name="defaultRoute">
                                    </LABEL></TD></TR></TBODY></TABLE></DIV>
<% end if %>
                  <DIV id=secIgmp>
                  <TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
                  ...
                  <TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
                    ...
                    <TR>
                      ...
                      <TD><INPUT id=pppServerName maxLength=63 size=15 name=pppServerName value="<%if TCWebApi_get("system_entry","srvName","h" ) <> "N/A" then TCWebApi_get("system_entry","srvName","s" ) end if %>" style="FONT-FAMILY: '<%tcWebApi_get("String_Entry","NewRomanText","s")%>'">
                                    </TD>
                                  </TR>
